...
pacman::p_load(ggplot2, gridExtra, RColorBrewer, reshape2)
...
Looking at the section of code below, I see it is looping over each of the values in `ii`, which holds a random sample of 4 predicted y means held in the list `yPredMu`. Inside the loop, we assign `i` to be the `j`th value in `ii`, and `wi` to be the index of the current `yPredMu` column. `zj` is this column's standard error divided by the mean with some error term equal to 0.01. We then run a custom Clark Function, `values2contour()`, which takes the latlon values from `predGrid` and creates a contour map. 

After creating the contour map, this chunk overlays the border of the region of interest (`maps::map` line). We then recover the inputted `y` data from the GJAM output `out`, scale it by 1000 hours, and then again by its maximum value in order to get reasonable scales for our plot point areas. Finally, this chunk plots the xdata's latitude and longitude points on top of the second map, using the scaled values in `ydata` as the radius for the various circles.
